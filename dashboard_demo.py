"""
Demo — A Historia da Recompra
Versao com dados mascarados para apresentacao a leads.
Duas abas: storytelling + dashboard analitico.
Dark-mode native.
"""
...
# ═══════════════════════════════════════
# MASKING — replace real names with generic labels
# ═══════════════════════════════════════

@st.cache_data
def _build_masks(df):
    """Build consistent mappings for all sensitive fields."""
    # Products: sorted by frequency (most sold first) for stable numbering
    prod_counts = df.groupby("seo_title")["numero"].nunique().sort_values(ascending=False)
    prod_map = {name: f"Produto {i+1}" for i, name in enumerate(prod_counts.index)}

    # Brands
    brands = sorted(df["marca"].dropna().unique())
    marca_map = {name: f"Marca {chr(65+i)}" for i, name in enumerate(brands)}

    # Sub-categories
    subcats = sorted(df["sub_categoria"].dropna().unique())
    subcat_map = {name: f"Categoria {i+1}" for i, name in enumerate(subcats)}

    # Parent categories
    cats = sorted(df["categoria_mae"].dropna().unique())
    cat_map = {name: f"Grupo {chr(65+i)}" for i, name in enumerate(cats)}

    return prod_map, marca_map, subcat_map, cat_map


def _mask_df(df, prod_map, marca_map, subcat_map, cat_map):
    """Apply masking to a dataframe (returns copy)."""
    d = df.copy()
    if "seo_title" in d.columns:
        d["seo_title"] = d["seo_title"].map(prod_map).fillna(d["seo_title"])
    if "marca" in d.columns:
        d["marca"] = d["marca"].map(marca_map).fillna(d["marca"])
    if "sub_categoria" in d.columns:
        d["sub_categoria"] = d["sub_categoria"].map(subcat_map).fillna(d["sub_categoria"])
    if "categoria_mae" in d.columns:
        d["categoria_mae"] = d["categoria_mae"].map(cat_map).fillna(d["categoria_mae"])
    return d


st.set_page_config(page_title="Demo — A Historia da Recompra", layout="wide")
...
<style>
    ...
    .demo-badge {
        ...
        background: rgba(249, 168, 37, 0.9); color: #000; padding: 6px 16px;
...
def tab_story(df, df_novo, fi, subs, clients, novos, gw, subcat, combos, taxa_global, masks):
    prod_map, marca_map, subcat_map, cat_map = masks
    total_novos = len(novos)
    total_recompra = clients["recomprou"].sum()

    # Mask dataframes for display
    gw_m = _mask_df(gw, prod_map, marca_map, subcat_map, cat_map)
    subcat_m = _mask_df(subcat, prod_map, marca_map, subcat_map, cat_map)
    combos_m = _mask_df(combos, prod_map, marca_map, subcat_map, cat_map) if len(combos) > 0 else combos
    ...
    gw_f = gw_m[gw_m["clientes_1a"] >= min_cli].copy()
    ...
    # ── 02: CATEGORIES ──
    section_header(
        "02 — CATEGORIAS QUE GERAM RECORRENCIA",
        "Categorias de consumo recorrente geram mais recompra que duraveis.",
    )

    sc_f = subcat_m[subcat_m["clientes"] >= 30].sort_values("taxa_recompra", ascending=True)
    fig2 = go.Figure()
    ...
    insight(
        ...
        f"Categorias de consumo recorrente fazem o cliente voltar."
    )
    ...
    if len(combos_m) > 0:
        # Re-mask combo column
        combos_disp = combos_m.copy()
        if "item_a" in combos_disp.columns:
            combos_disp["item_a"] = combos_disp["item_a"].map(subcat_map).fillna(combos_disp["item_a"])
            combos_disp["item_b"] = combos_disp["item_b"].map(subcat_map).fillna(combos_disp["item_b"])
            combos_disp["combo"] = combos_disp["item_a"] + "  +  " + combos_disp["item_b"]

        combos_f = combos_disp[combos_disp["clientes"] >= 30].copy()
    ...
    insight(
        ...
        "Melhores candidatos para investir em aquisicao."
    )
    ...
    section_header(
        "05 — MIDIA PAGA vs VENDAS REAIS",
        "Cruzamos spend de midia com clientes NOVOS do ERP (nao conversoes do Google). "
        "CAC = quanto gastou em Ads / quantos clientes novos entraram naquele estado x marca.",
    )

    cross_raw = load_ads_and_cross(df)
    # Mask marca in cross data
    cross_raw_m = cross_raw.copy()
    cross_raw_m["marca"] = cross_raw_m["marca"].map(marca_map).fillna(cross_raw_m["marca"])

    rec_uf, rec_uf_marca, rec_marca = build_recompra_by_uf_marca(fi, clients, novos)
    rec_marca_m = rec_marca.copy()
    rec_marca_m["marca"] = rec_marca_m["marca"].map(marca_map).fillna(rec_marca_m["marca"])
    rec_uf_marca_m = rec_uf_marca.copy()
    rec_uf_marca_m["marca"] = rec_uf_marca_m["marca"].map(marca_map).fillna(rec_uf_marca_m["marca"])

    # ── Date filter ──
    all_months = sorted(cross_raw_m["mes"].unique())
    ...
    cross = cross_raw_m[cross_raw_m["mes"] == sel_month].copy()
    ...
    cr_marca = cr_marca.merge(rec_marca_m[["marca", "taxa_recompra", "ltv_medio"]], on="marca", how="left")
    cr_marca = cr_marca[cr_marca["clientes_novos"] >= 3]

    st.markdown("**Marca: CAC (Custo de Aquisicao de Cliente) vs Taxa de Recompra**")
    st.caption("CAC = spend Ads / clientes NOVOS no ERP. Bolha = spend. Ideal = canto inferior direito.")
    ...
    # Use masked gw for display, but keep original for data lookups
    gw_orig = gw[gw["clientes_1a"] >= min_cli].copy()
    gw_dedup = gw_f.sort_values("clientes_1a", ascending=False).drop_duplicates("seo_title", keep="first")

    # Masked fi/subs for product detail
    fi_m = _mask_df(fi, prod_map, marca_map, subcat_map, cat_map)
    subs_m = _mask_df(subs, prod_map, marca_map, subcat_map, cat_map)

    vendas_feb = (
        _mask_df(df, prod_map, marca_map, subcat_map, cat_map)[
            _mask_df(df, prod_map, marca_map, subcat_map, cat_map)["mes"] == "2026-02"
        ]
        .groupby("seo_title")
    ...
    if selected:
        _render_product_detail(selected, gw_f, fi_m, subs_m, clients, novos, taxa_global)

    # ── FOOTER ──
    st.markdown('<hr class="divider">', unsafe_allow_html=True)
    st.markdown(
        '<div class="footer-text">'
        'Analise de Recompra · Apenas clientes novos · Exclui cancelados<br>'
...
def tab_dashboard(df, df_novo, fi, subs, clients, novos, gw, subcat, combos, taxa_global, masks):
    prod_map, marca_map, subcat_map, cat_map = masks
    total_novos = len(novos)
    total_recompra = clients["recomprou"].sum()
    ...
    with c2:
        marcas = st.multiselect("Marcas", sorted(gw_m["marca"].dropna().unique()), key="marcas_dash")
    with c3:
        cats = st.multiselect("Grupos", sorted(gw_m["categoria_mae"].dropna().unique()), key="cats_dash")
    ...
    sc_f = subcat_m[subcat_m["clientes"] >= 20].sort_values("taxa_recompra", ascending=False)
    st.dataframe(
        sc_f.style.format({
            "taxa_recompra": "{:.1%}",
            "media_orders": "{:.1f}",
    ...
    if len(combos_m) > 0:
        ...
        combos_f = combos_disp[combos_disp["clientes"] >= 30].sort_values("uplift", ascending=False)
        st.dataframe(
    ...
    st.markdown("### Midia Paga x Vendas Reais (ERP)")
    cross_raw = load_ads_and_cross(df)
    cross_raw_m = cross_raw.copy()
    cross_raw_m["marca"] = cross_raw_m["marca"].map(marca_map).fillna(cross_raw_m["marca"])
    ...
    cross = cross_raw_m[cross_raw_m["mes"] == sel_month_d]
    ...
    cr_full = cr_full.merge(rec_uf_marca_m[["uf", "marca", "taxa_recompra"]], on=["uf", "marca"], how="left")
    ...
    df_m = _mask_df(df, prod_map, marca_map, subcat_map, cat_map)
    vendas_feb2 = (
        df_m[df_m["mes"] == "2026-02"]
        .groupby("seo_title")
        .agg(pedidos_feb=("numero", "nunique"))
        .reset_index()
    )
    ...
    if options:
        ...
        if selected:
            _render_product_detail(selected, gw_f, fi_m, subs_m, clients, novos, taxa_global, prefix="d")
...
def main():
    ...
    # Build masks from raw data
    masks = _build_masks(df)

    tab1, tab2 = st.tabs(["A Historia", "Dashboard"])

    with tab1:
        tab_story(df, df_novo, fi, subs, clients, novos, gw, subcat, combos, taxa_global, masks)

    with tab2:
        tab_dashboard(df, df_novo, fi, subs, clients, novos, gw, subcat, combos, taxa_global, masks)
